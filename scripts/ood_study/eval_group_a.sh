...
sbatch --job-name="eval-a1" --qos=$QOS --gres=gpu:$GPUS --cpus-per-task=$CPUS --mem=$MEM --time=$TIME \
    ...
    --chdir="$PWD" --wrap="export TORCH_COMPILE_DISABLE=1 && python scripts/eval_beir.py --model $COMMON_DIR/m5-colbert-seed1/final --document_length 512 --dataset all"
...
sbatch --job-name="eval-a2zs" --qos=$QOS --gres=gpu:$GPUS --cpus-per-task=$CPUS --mem=$MEM --time=$TIME \
    ...
    --chdir="$PWD" --wrap="export TORCH_COMPILE_DISABLE=1 && python scripts/ood_study/eval_colbert_beir.py --model $COMMON_DIR/m5-colbert-seed1/final --aggregation mean --document_length 512 --dataset all"
...
sbatch --job-name="eval-a2nat" --qos=$QOS --gres=gpu:$GPUS --cpus-per-task=$CPUS --mem=$MEM --time=$TIME \
    ...
    --chdir="$PWD" --wrap="export TORCH_COMPILE_DISABLE=1 && python scripts/ood_study/eval_colbert_beir.py --model $COMMON_DIR/a2nat-meansim-seed1/final --aggregation mean --document_length 512 --dataset all"
...
sbatch --job-name="eval-a4" --qos=$QOS --gres=gpu:$GPUS --cpus-per-task=$CPUS --mem=$MEM --time=$TIME \
    ...
    --chdir="$PWD" --wrap="export TORCH_COMPILE_DISABLE=1 && python scripts/ood_study/eval_dense_beir.py --model $COMMON_DIR/m2-dense-seed1/final --dataset all"
